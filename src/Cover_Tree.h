/* https://github.com/DNCrane/Cover-Tree/commit/b6de5de9cafcc496f640dd8ddc63e061d0226519
...
class CoverTree
{
 ...
 private:
    ...
    std::set<distNodePair>
        kNearestNodeSet(const Point& p, const unsigned int& k) const;
 ...
 public:
    ...
    std::vector<std::pair<double, Point>> kNearestNeighborDists(const Point& p, const unsigned int& k) const;
...
template<class Point>
std::set<std::pair<double, typename CoverTree<Point>::CoverTreeNode*>>
CoverTree<Point>::kNearestNodeSet(const Point& p, const unsigned int& k) const
{
    if(_root==NULL) return std::set<distNodePair>();
